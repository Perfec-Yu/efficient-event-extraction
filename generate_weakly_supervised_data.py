...
from typing import *
from utils.utils import WSAnnotations, get_dataset_and_loader, F1MetricTag
...
def _dist_tokenize_and_lemmatize_corpus() -> Callable:
    ...
    def _step_doc(doc, lemmatizer):
        tokens = pos_tag(word_tokenize(doc[1]), tagset='universal')
        lemmas = [lemmatizer.lemmatize(token, 'v') if pos == 'VERB' else lemmatizer.lemmatize(token, 'n') for token, pos in tokens]
        return {
            "tokens": [token for token, _ in tokens],
            "pos_tags": [pos for _, pos in tokens],
...
def _dist_create_tokenizer_and_process_corpus(id:int, queue:Queue, progress:Value, tokenized_corpus:List[Tokenized], model_name:str)-> None:
    ...
    for tokenized in tokenized_corpus:
        tokens = tokenized[1]
        encodings.append(tokenizer(tokens, add_special_tokens=True, is_split_into_words=True))
        progress.value = progress.value + 1
...
def keyword_matching(id:int, encodings:List[BatchEncoding], keyword_vectors:torch.FloatTensor, keywordid2label:Dict[int, str], label2id:Dict[str, int], th:float, uth:float, model_name:Optional[str]=None, tokenizer:Optional[PreTrainedTokenizerFast]=None, model:Optional[PreTrainedModel]=None, batch_size:int=16):
    if model_name is not None:
        if tokenizer is None:
            tokenizer = AutoTokenizer.from_pretrained(model_name)
        if model is None:
        ...
        raise ValueError("need either model name or loaded model/tokenizer")
    model = model.cuda(f"cuda:{id}")
    annotator = WSAnnotations(th, uth, tokenizer, label2id, keywordid2label)
    with torch.no_grad():
        ...
        for ibatch in tqdm(range(0, len(encodings), batch_size)):
            ...
            batch_encoding = batch_encoding.to(keyword_vectors.device)
            output = model(**batch_encoding).last_hidden_state
...
def evaluate_vectors_on_corpus(threshold:float, keyword_vectors:torch.FloatTensor, label2id:Dict[str, int], corpus_path:Optional[str]=None, model_name:Optional[str]=None, method:str="token", keywordid2labelid:Optional[Dict[int,int]]=None, model:Optional[PreTrainedModel]=None, dataloader:Optional[torch.utils.data.DataLoader]=None) -> Dict[str, float]:
    ...
    return {"precision": result.full_result[0], "recall": result.full_result[1], "f1": result.full_result[2]}
...
def main():
    ...
    with open(args.label_json, "rt") as fp:
        ...
        label_keywords = _clean_keywords({label:list(set(info["expanded_keywords"]+info["seed_keywords"])) for label, info in label_info.items()})
    ...
    if args.force or not os.path.exists(os.path.join(args.output_save_dir, f"keyword_mask_prediction.json")):
        ...
        prediction = _mask_prediction(0, prepared_corpus, occurences, "bert-large-cased", tokenizer=tokenizer)
    ...
    if args.force or not os.path.exists(os.path.join(args.output_save_dir, f"keyword_vector.th")):
        ...
        keyword2id = {"NA": 0}
        keyword_vectors = torch.zeros(len(keyword_vectors_dict), 1024, requires_grad=False)
        label_vectors = torch.zeros(len(label2id) - 1, 1024, requires_grad=False)
        ...
        cluster_vectors = torch.zeros(sum(t['ncluster'] for t in label_clusters.values()), 1024)
    ...
    if args.force or not os.path.exists(os.path.join(args.output_save_dir, f"weakly_supervised_data_kw.jsonl")):
        ...
        if model is None:
            model = AutoModel.from_pretrained(args.model_name)
        annotations = keyword_matching(0, prepared_corpus, keyword_vectors, keywordid2label, label2id, th=best_th['kw'], uth=best_th['kw'], model_name=args.model_name, model=model, tokenizer=tokenizer)
    ...
    if args.force or not os.path.exists(os.path.join(args.output_save_dir, f"weakly_supervised_data_cluster.jsonl")):
        ...
        if model is None:
            model = AutoModel.from_pretrained(args.model_name)
        annotations = keyword_matching(0, prepared_corpus, cluster_vectors, clusterid2label, label2id, th=best_th['cluster'], uth=best_th['kw'], model_name=args.model_name, model=model, tokenizer=tokenizer)
    ...
    if args.force or not os.path.exists(os.path.join(args.output_save_dir, f"weakly_supervised_data_label.jsonl")):
        ...
        if model is None:
            model = AutoModel.from_pretrained(args.model_name)
        annotations = keyword_matching(0, prepared_corpus, label_vectors, None, label2id, th=best_th['label'], uth=best_th['label'], model_name=args.model_name, model=model, tokenizer=tokenizer)
    ...
    if args.force or not os.path.exists(os.path.join(args.output_save_dir, f"weakly_supervised_dev_kw.jsonl")):
        ...
        if model is None:
            model = AutoModel.from_pretrained(args.model_name)
        annotations = keyword_matching(0, prepared_dev_corpus, keyword_vectors, keywordid2label, label2id, th=best_th['kw'], uth=best_th['kw'], model_name=args.model_name, model=model, tokenizer=tokenizer)
    ...
    if args.force or not os.path.exists(os.path.join(args.output_save_dir, f"weakly_supervised_dev_cluster.jsonl")):
        ...
        if model is None:
            model = AutoModel.from_pretrained(args.model_name)
        annotations = keyword_matching(0, prepared_dev_corpus, cluster_vectors, clusterid2label, label2id, th=best_th['cluster'], uth=best_th['cluster'], model_name=args.model_name, model=model, tokenizer=tokenizer)
    ...
    if args.force or not os.path.exists(os.path.join(args.output_save_dir, f"weakly_supervised_dev_label.jsonl")):
        ...
        if model is None:
            model = AutoModel.from_pretrained(args.model_name)
        annotations = keyword_matching(0, prepared_dev_corpus, label_vectors, None, label2id, th=best_th['label'], uth=best_th['label'], model_name=args.model_name, model=model, tokenizer=tokenizer)
