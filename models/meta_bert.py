from torch.nn.functional import gelu, elu, pairwise_distance
...
def functional_bert(fast_weights, config, input_ids=None, attention_mask=None, token_type_ids=None,
    ...
    outputs = (sequence_output,sequence_output[:, 0, :])
    return outputs
...
def functional_embeeding(fast_weights, config, input_ids, position_ids, 
    ...
    if inputs_embeds is None:
        inputs_embeds = F.embedding(input_ids, fast_weights['embeddings.word_embeddings.weight'], padding_idx = 0)
    
    position_embeddings = F.embedding(position_ids, fast_weights['embeddings.position_embeddings.weight'])
    token_type_embeddings = F.embedding(token_type_ids, fast_weights['embeddings.token_type_embeddings.weight'])

    embeddings = inputs_embeds + position_embeddings + token_type_embeddings
    
    embeddings = F.layer_norm(embeddings, [config.hidden_size], 
                              weight=fast_weights['embeddings.LayerNorm.weight'],
                              bias=fast_weights['embeddings.LayerNorm.bias'],
...
def functional_self_attention(fast_weights, config, layer_idx,
    ...
    mixed_query_layer = F.linear(hidden_states,
                                fast_weights['encoder.layer.'+layer_idx+'.attention.self.query.weight'],
                                fast_weights['encoder.layer.'+layer_idx+'.attention.self.query.bias'])
    
    if encoder_hidden_states is not None:
        mixed_key_layer = F.linear(encoder_hidden_states,
                                fast_weights['encoder.layer.'+layer_idx+'.attention.self.key.weight'],
                                fast_weights['encoder.layer.'+layer_idx+'.attention.self.key.bias'])
        mixed_value_layer = F.linear(encoder_hidden_states,
                                fast_weights['encoder.layer.'+layer_idx+'.attention.self.value.weight'],
                                fast_weights['encoder.layer.'+layer_idx+'.attention.self.value.bias'])
...
def functional_out_attention(fast_weights, config, layer_idx,
    ...
    hidden_states = F.linear(hidden_states,
                            fast_weights['encoder.layer.'+layer_idx+'.attention.output.dense.weight'],
                            fast_weights['encoder.layer.'+layer_idx+'.attention.output.dense.bias'])
    ...
    hidden_states = F.layer_norm(hidden_states + input_tensor, [config.hidden_size],
                              weight=fast_weights['encoder.layer.'+layer_idx+'.attention.output.LayerNorm.weight'],
                              bias=fast_weights['encoder.layer.'+layer_idx+'.attention.output.LayerNorm.bias'],
                              eps=config.layer_norm_eps)
...
def functional_intermediate(fast_weights, config, layer_idx, hidden_states, is_train = True):
    weight_name = 'encoder.layer.' + layer_idx + '.intermediate.dense.weight'
    bias_name   = 'encoder.layer.' + layer_idx + '.intermediate.dense.bias'
...
def functional_output(fast_weights, config, layer_idx, hidden_states, input_tensor, is_train = True):
    ...
    hidden_states = F.linear(hidden_states, 
                             fast_weights['encoder.layer.'+layer_idx+'.output.dense.weight'], 
                             fast_weights['encoder.layer.'+layer_idx+'.output.dense.bias'])
    
    hidden_states = F.dropout(hidden_states, p=config.hidden_dropout_prob, training = is_train)
    hidden_states = F.layer_norm(hidden_states + input_tensor, [config.hidden_size],
                              weight=fast_weights['encoder.layer.'+layer_idx+'.output.LayerNorm.weight'],
                              bias=fast_weights['encoder.layer.'+layer_idx+'.output.LayerNorm.bias'],
                              eps=config.layer_norm_eps)
    return hidden_states
...
class MetaBertModel(BertModel, MetaModule):
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None,
                    position_ids=None, head_mask=None, inputs_embeds=None, encoder_hidden_states=None,
                    encoder_attention_mask=None, is_train = True, params=None):
        if params is None: params = OrderedDict(self.named_parameters())
        return functional_bert(
            fast_weights=params,
            config=self.config,
            input_ids=input_ids, 
